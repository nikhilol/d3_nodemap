...
export function Signup() {
    ...
    async function signupHandler() {
        if (document.getElementById('Password').value === document.getElementById('RepeatPassword').value) {
            try {
                const res = await axios.post('http://localhost:5001/nodemap-app/us-central1/api/signup', { Email: document.getElementById('Email').value, Username: document.getElementById('Username').value, Password: document.getElementById('Password').value }).catch(err => alert(err))
                console.log(res)
